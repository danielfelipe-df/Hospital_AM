...
void main_trace(std::vector<grupo> &Val, std::vector<grupo> &Vba, trabajadores *altos, trabajadores *bajos, double time, Crandom &ran){
  ...
  aux_main(num, Val, Vba, altos, bajos, ran, phi1, mu, true, 11);

  num = tested_isolated_inf(Vba[10], Vba[11], Vba[9], bajos, time, 10, 11, 9, ran); //Leves bajos
  aux_main(num, Val, Vba, altos, bajos, ran, mu, chi, false, 11);
}
...
int reaction_trace(std::vector<grupo> &V, trabajadores *family, Crandom &ran, int index){
  ...
  else if(family[index].kind == 8){aux_trace(V[8], V[7], family, 8, 7, index, true);    return 1;} //Presintomático masivo
  else if(family[index].kind == 9){aux_trace(V[9], V[11], family, 9, 11, index, true);    return 1;} //Leve
  else if(family[index].kind == 10){aux_trace(V[10], V[11], family, 10, 11, index, true);    return 1;} //Leve testeado
  else if(family[index].kind == 12){aux_trace(V[12], V[11], family, 12, 11, index, true);    return 1;} //Leve masivo
  else if(family[index].kind == 14){aux_trace(V[14], V[15], family, 14, 15, index, true);    return 1;} //Recuperado no-detectado
  else if(family[index].kind == 15){aux_trace(V[15], V[15], family, 14, 15, index, false);    return 1;} //Recuperado testeado
  else{return 0;}
}
