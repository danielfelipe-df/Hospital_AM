...
int reaction_trace(std::vector<grupo> &V, trabajadores *family, Crandom &ran, int index){
  ...
  else if(family[index].kind == 8){aux_trace(V[8], V[10], family, 8, 10, index, true);    return 1;} //Leve
  else if(family[index].kind == 9){aux_trace(V[9], V[10], family, 9, 10, index, true);    return 1;} //Leve testeado
