...
typedef void(*reactions) (grupo &Sa, grupo &Sb, grupo &Ea, grupo &Eb, grupo &Pa, grupo &Pb, grupo &PTa, grupo &PTb, grupo &PTAa, grupo &PTAb, grupo &La, grupo &Lb, grupo &LTa, grupo &LTb, grupo &LTAa, grupo &LTAb, grupo &IAa, grupo &IAb, grupo &RIa, grupo &RIb, grupo &RAa, grupo &RAb, Crandom &ran, trabajadores *altos, trabajadores *bajos, std::vector<weib_d> &dist);
...
int main(void)
{
  ...
  for(unsigned int i=0; i<ensemble; i++){
	...
	ti_in = contagio(susal.size(), susba.size(), expal.size(), expba.size(), preal.size(), preba.size(), preTal.size(), preTba.size(), preTAal.size(), preTAba.size(), leval.size(), levba.size(), levTal.size(), levTba.size(), levTAal.size(), levTAba.size(), infAal.size(), infAba.size(), Na, Nb, prev, gseed, t, tj);
	...
	react[(int)ti_in[1]](susal, susba, expal, expba, preal, preba, preTal, preTba, preTAal, preTAba, leval, levba, levTal, levTba, levTAal, levTAba, infAal, infAba, recIal, recIba, recAal, recAba, gseed, altos, bajos, dist);
	...
	for(unsigned int k=n1; k<n2 && k<tests; k++){
	  if(gseed.r()*N < Na){massive_reaction(susal, expal, preal, preTal, leval, levTal, recIal, gseed, altos);}
	  else{massive_reaction(susba, expba, preba, preTba, levba, levTba, recIba, gseed, bajos);}
	}
	n1 = n2;
    ...
    recIal.clear();    recIba.clear();
    recAal.clear();    recAba.clear();
  }
